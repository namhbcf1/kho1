...
class UploadService {
  async uploadImage(storage: R2Bucket, file: File): Promise<UploadResponse> {
    try {
      // Validate file type
  ...
  async uploadAvatar(storage: R2Bucket, file: File, userId: string): Promise<UploadResponse> {
    try {
      // Validate file type
      const allowedTypes = ['image/jpeg', 'image/png', 'image/webp'];
      if (!allowedTypes.includes(file.type)) {
  ...
  async deleteFile(storage: R2Bucket, filename: string): Promise<UploadResponse> {
    try {
