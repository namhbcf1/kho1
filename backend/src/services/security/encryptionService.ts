...
export interface EncryptionKey {
  ...
  expiresAt?: Date;
  isActive: boolean;
  purpose: 'general' | 'payment' | 'personal' | 'audit';
}
...
export class EncryptionService {
  ...
  private async initializeEncryption(): Promise<void> {
    try {
      ...
      const purposes: EncryptionKey['purpose'][] = ['general', 'payment', 'personal', 'audit'];
   ...
   */
  async encryptAuditData(auditData: any): Promise<EncryptedData> {
   ...
   */
  async rotateKey(purpose: EncryptionKey['purpose']): Promise<KeyRotationResult> {
    try {
   ...
   */
  async hashForSearch(data: string, salt?: string): Promise<string> {
    const encoder = new TextEncoder();
   ...
   */
  async validateEncryptedData(encryptedData: EncryptedData): Promise<{
    isValid: boolean;
    issues: string[];
  }> {
    const issues: string[] = [];
    ...
    if (!encryptedData.data) issues.push('Missing encrypted data');
    if (!encryptedData.iv) issues.push('Missing initialization vector');
    if (!encryptedData.tag) issues.push('Missing authentication tag');
    if (!encryptedData.keyId) issues.push('Missing key identifier');
  ...
  getServiceHealth(): {
    ...
    nextRotations: Array<{ purpose: string; scheduledAt: Date }>;
    issues: string[];
  } {
    const issues: string[] = [];
    ...
    const nextRotations = Array.from(this.keyRotationSchedule.entries()).map(([purpose, timeout]) => ({
      purpose,
      scheduledAt: new Date(Date.now() + (timeout as any)._idleTimeout)
    }));
  ...
  private async generateNewKey(purpose: EncryptionKey['purpose']): Promise<EncryptionKey> {
  ...
  private scheduleKeyRotation(purpose: EncryptionKey['purpose']): void {
