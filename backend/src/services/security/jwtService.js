...
export class JWTAuthenticationService {
    ...
    auditLogs = [];
    constructor(db, encryptionService, config) {
        this.db = db;
     ...
     */
    async generateTokenPair(userClaims, ipAddress, userAgent) {
        try {
    ...
    async validateAccessToken(token) {
        try {
            ...
            const payload = await this.verifyToken(token, this.config.accessTokenSecret);
    ...
    async refreshAccessToken(refreshToken, ipAddress, userAgent) {
        try {
            ...
            const refreshPayload = await this.verifyToken(refreshToken, this.config.refreshTokenSecret);
    ...
    async revokeToken(token) {
        try {
            ...
            const payload = await this.verifyToken(token, this.config.accessTokenSecret, false);
            if (payload) {
    ...
    getServiceHealth() {
        ...
        const recentAlerts = this.auditLogs.filter(log => log.riskLevel === 'high' || log.riskLevel === 'critical').length;
        let status = 'healthy';
        if (recentAlerts > 10) {
            status = 'critical';
        }
    ...
    async verifyToken(token, secret, requireValid = true) {
    ...
    `).bind(token.tokenId, token.userId, token.sessionId, token.deviceId, token.ipAddress, token.userAgent, token.createdAt.toISOString(), token.expiresAt.toISOString(), token.lastUsed.toISOString(), token.isRevoked ? 1 : 0).run();
    }
    async getStoredRefreshToken(tokenId) {
    ...
    async updateRefreshToken(token) {
      ...
      WHERE token_id = ?
    `).bind(token.lastUsed.toISOString(), token.isRevoked ? 1 : 0, token.tokenId).run();
    }
    ...
    `).bind(event.timestamp.toISOString(), event.action, event.userId, event.sessionId, event.ipAddress, event.userAgent, event.success ? 1 : 0, JSON.stringify(event.details), event.riskLevel).run();
    }
    ...
    async cleanupExpiredTokens() {
        ...
        this.auditLogs = this.auditLogs.filter(log => (now.getTime() - log.timestamp.getTime()) < 30 * 24 * 60 * 60 * 1000 // 30 days
        );
    }
}
/**
