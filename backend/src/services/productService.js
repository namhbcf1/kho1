class ProductService {
    ...
    async getProductById(db, productId) {
        try {
    ...
    async createProduct(db, productData) {
        ...
        `)
                .bind(productId, productData.name, productData.description || null, productData.price, productData.cost || null, productData.stock, productData.minStock || null, productData.barcode || null, productData.sku || null, productData.categoryId, productData.images ? JSON.stringify(productData.images) : null, productData.active ? 1 : 0)
                .run();
    ...
    async updateProduct(db, productId, updateData) {
        try {
            const updates = [];
            const values = [];
    ...
    async deleteProduct(db, productId) {
        try {
            await db
