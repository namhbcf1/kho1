...
export class EventStore {
  ...
  async appendToStream(
    aggregateId: string,
    aggregateType: string,
    events: Omit<DomainEvent, 'id' | 'streamPosition'>[],
  ...
  async readStream(
    ...
    const currentVersion = events.results.length > 0 
      ? Math.max(...events.results.map((e: any) => e.stream_position))
      : 0;
   ...
   */
  async getEventsAfterPosition(position: number, limit = 100): Promise<DomainEvent[]> {
    const events = await this.db.prepare(`
      SELECT id, aggregate_id, aggregate_type, event_type, 
             event_data, metadata, stream_position, created_at
      FROM event_store 
    ...
    return events.results.map((row: any) => ({
      id: row.id,
      aggregateId: row.aggregate_id,
      aggregateType: row.aggregate_type,
      eventType: row.event_type,
...
export abstract class AggregateRoot {
   ...
   */
  protected addEvent(eventType: string, eventData: Record<string, any>, metadata: any): void {
  ...
  /**
   * Apply event to change state (must be implemented by subclasses)
   */
...
export class FinancialTransaction extends AggregateRoot {
  ...
  completePayment(gatewayTransactionId: string, userId: string): void {
  ...
  failPayment(reason: string, userId: string): void {
    if (this.status !== 'pending') {
      throw new Error('Transaction is not in pending status');
    }
  ...
  refundPayment(refundAmount: number, reason: string, userId: string): void {
...
export class EventSourcedRepository<T extends AggregateRoot> {
  constructor(
    ...
    private createAggregate: (id: string) => T
  ) {}

  async save(aggregate: T): Promise<{ success: boolean; error?: string }> {
