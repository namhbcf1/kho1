...
class EnhancedAuthService {
  ...
  async register(
    ...
    try {
      ...
      await db
        ...
        `)
        .bind(
          userId, validatedData.email.toLowerCase(), hashedPassword, 
          validatedData.name, validatedData.role,
          validatedData.phone || null, validatedData.position || null,
          now, now, now
        )
        .run();
  ...
  async refreshTokens(
    ...
    try {
      ...
      const userResult = await this.getUserById(db, tokenRecord.user_id as string);
      ...
      await invalidateSession(db, tokenRecord.session_id as string);

      // Generate new tokens
      const tokens = await generateAuthTokens(
        userResult.user, 
        env, 
        deviceId || tokenRecord.device_id as string,
        ipAddress
      );
  ...
  async cleanupExpired(db: D1Database): Promise<void> {
