...
export interface VietnamInvoiceData {
  // Invoice metadata
  ...
  compliance: {
    retentionPeriod: number; // 5 years for Vietnamese law
...
export class VietnameseInvoiceService {
   ...
   */
  static async generateInvoice(params: z.infer<typeof InvoiceCreateSchema>): Promise<VietnamInvoiceData> {
    ...
    const vatGroups = new Map<number, { taxableAmount: number; taxAmount: number }>();
    ...
    // Create invoice data
    const invoiceData: VietnamInvoiceData = {
      invoiceNumber,
      invoiceDate,
   ...
   */
  static async generateReceipt(invoiceData: VietnamInvoiceData): Promise<{
    receiptNumber: string;
    receiptData: any;
    thermalPrintData: string;
  }> {
   ...
   */
  static generateTaxReportByCategory(
    invoices: VietnamInvoiceData[],
    ...
    // Group by VAT rate
    const vatRateGroups = new Map<number, {
      count: number;
      taxableAmount: number;
      vatAmount: number;
    }>();
   ...
   */
  static validateInvoiceCompliance(invoiceData: VietnamInvoiceData): {
    isValid: boolean;
    errors: string[];
    warnings: string[];
  } {
  ...
  private static generateInvoiceNumber(type: string): string {
    const date = new Date();
  ...
  private static getInvoiceTemplate(type: string): string {
    return {
  ...
  private static getInvoiceForm(): string {
    return 'Mẫu số: 01-HĐ/2024';
  }
  
  private static getInvoiceSymbol(type: string): string {
    return {
      retail: 'AA/24E',
      wholesale: 'BB/24E',
      export: 'CC/24E',
  ...
  private static generateThermalPrintData(receiptData: any): string {
