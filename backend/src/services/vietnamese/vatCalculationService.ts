...
export interface ProductVATCategory {
  categoryId: string;
  categoryName: string;
  vatType: VATRate['type'];
  isEssential: boolean; // Essential goods may have reduced VAT
  requiresDeclaration: boolean; // Some items require special VAT declarations
}
...
export class VietnameseVATService {
  ...
  private productCategories: Map<string, ProductVATCategory> = new Map();
  ...
  calculateOrderVAT(
    ...
    for (const item of items) {
      const itemResult = this.calculateItemVAT(
        item.productId,
        item.preTaxPrice,
        item.quantity,
        customerType
    ...
    let dominantVATType: VATRate['type'] = 'standard';
  ...
  generateVATDeclaration(
    ...
    const declarations: VATDeclaration['declarations'] = [];
    let totalRevenue = 0;
    let totalVATCollected = 0;

    for (const transaction of transactions) {
      for (const item of transaction.items) {
        const vatResult = this.calculateItemVAT(
          item.productId,
          item.preTaxPrice,
          item.quantity,
   ...
   */
  validateCompliance(vatResult: VATCalculationResult): {
    isCompliant: boolean;
    violations: string[];
   ...
   */
  getCurrentVATRates(): Array<VATRate & { categoryCount: number }> {
    const rates: Array<VATRate & { categoryCount: number }> = [];
    
    for (const [type, rate] of this.vatRates) {
      const categoryCount = Array.from(this.productCategories.values())
        .filter(cat => cat.vatType === type as VATRate['type']).length;
   ...
   */
  updateProductVATCategory(
    productId: string,
    category: Omit<ProductVATCategory, 'categoryId'>
  ): void {
  ...
  private initializeVATRates(): void {
  ...
  private getVATRate(type: VATRate['type']): VATRate {
  ...
  private getComplianceRequirements(
    ...
    amount: number,
    customerType: 'individual' | 'business'
  ): VATCalculationResult['compliance'] {
    const notes: string[] = [];
