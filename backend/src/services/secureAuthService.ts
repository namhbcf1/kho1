...
import { sign, verify, JwtPayload } from '@cloudflare/itty-jwt';
...
export class SecureAuthService {
  private readonly ACCESS_TOKEN_EXPIRY = 24 * 60 * 60; // 24 hours
  private readonly REFRESH_TOKEN_EXPIRY = 7 * 24 * 60 * 60; // 7 days
  private readonly CSRF_TOKEN_EXPIRY = 60 * 60; // 1 hour
  ...
  async login(
    ...
    ipAddress: string,
    userAgent: string
  ): Promise<{ tokens: SecureTokens; user: UserSession } | { error: string; blocked?: boolean }> {
    try {
  ...
  async refreshTokens(
    ...
    ipAddress: string,
    userAgent: string
  ): Promise<{ tokens: SecureTokens } | { error: string }> {
    try {
   ...
   * Generate CSRF token
   */
  private async generateCsrfToken(userId: number): Promise<string> {
    ...
    await this.kv.put(
      `csrf:${userId}:${token}`,
      expiresAt.toString(),
      { expirationTtl: this.CSRF_TOKEN_EXPIRY }
    );
   ...
   * Verify CSRF token
   */
  private async verifyCsrfToken(token: string, userId: number): Promise<boolean> {
   ...
   * Check for brute force attacks
   */
  private async checkBruteForce(email: string, ipAddress: string): Promise<{ blocked: boolean; attempts: number }> {
   ...
   */
  async logout(sessionId: string, userId: number, ipAddress: string, userAgent: string): Promise<void> {
