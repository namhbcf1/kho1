export class TransactionService {
    ...
    async updateInventoryAtomic(updateData) {
        return this.withRetry(async () => {
            ...
            const currentVersion = currentProduct.version || 0;
            let newStock;
            switch (operation) {
                case 'add':
        ...
        `).bind(crypto.randomUUID(), productId, operation === 'add' ? 'in' : 'out', operation === 'add' ? quantity : -quantity, currentStock, newStock, orderId ? 'order' : 'adjustment', orderId || crypto.randomUUID(), reason || `Stock ${operation}`, userId || 'system'));
            // Execute transaction
    ...
    async createOrderAtomic(orderData) {
        ...
        `).bind(orderId, orderData.orderNumber, orderData.customerId || null, orderData.cashierId, orderData.subtotal, orderData.discount, orderData.tax, orderData.total, orderData.paymentMethod, orderData.notes || null));
          ...
          `).bind(item.quantity, item.productId, stockCheck.version, item.quantity));
          ...
          `).bind(crypto.randomUUID(), item.productId, -item.quantity, stockCheck.currentStock, stockCheck.currentStock - item.quantity, orderId, orderData.cashierId));
            }
    ...
    async reserveInventory(productId, quantity, orderId, expiresInMinutes = 30) {
        return this.withRetry(async () => {
            const statements = [];
            // Check current stock
            const product = await this.db
          ...
          WHERE id = ? AND version = ?
        `).bind(quantity, productId, product.version || 0));
        ...
        `).bind(crypto.randomUUID(), productId, orderId, quantity));
            const results = await this.db.batch(statements);
    ...
    async releaseInventoryReservation(orderId) {
        return this.withRetry(async () => {
          ...
          `).bind(quantity, productId));
            ...
            statements.push(this.db.prepare('DELETE FROM inventory_reservations WHERE order_id = ?')
                .bind(orderId));
            const results = await this.db.batch(statements);
            const allSucceeded = results.every(result => result.success);
            if (!allSucceeded) {
    ...
    async cleanupExpiredReservations() {
        return this.withRetry(async () => {
          ...
          `).bind(quantity, productId));
            ...
            statements.push(this.db.prepare('DELETE FROM inventory_reservations WHERE expires_at < datetime("now")'));
            const results = await this.db.batch(statements);
            const allSucceeded = results.every(result => result.success);
            if (!allSucceeded) {
                return {
