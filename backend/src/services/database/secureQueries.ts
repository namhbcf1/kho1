...
export class SecureQueryBuilder {
  ...
  async searchProducts(params: z.infer<typeof productSearchSchema>) {
  ...
  async createProduct(productData: z.infer<typeof productCreateSchema>) {
  ...
  async updateProduct(productId: string, updateData: z.infer<typeof productUpdateSchema>) {
  ...
  // Secure stock update
  async updateStock(productId: string, stockData: z.infer<typeof stockUpdateSchema>) {
    ...
    try {
      ...
      const currentStock = (product.data as any).stock || 0;
      let newStock: number;
  ...
  private async executeQuery<T>(query: string, params: any[] = []): Promise<{
    success: boolean;
    data?: T;
    error?: string;
    message?: string;
...
export function validateAndSanitizeId(id: string): { isValid: boolean; sanitizedId?: string; error?: string } {
