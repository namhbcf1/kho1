...
import { Schemas, Product, ProductCreate, ProductUpdate, Search } from './schemas';
import { z } from 'zod';
...
export class ProductService {
  ...
  async getProducts(searchParams: Search) {
    try {
      const validatedParams = Schemas.Search.parse(searchParams);
      
      let whereConditions: string[] = ['p.status = ?'];
      ...
      const countResult = await this.d1Service.query<{ total: number }>(countSql, params);
      ...
      const result = await this.d1Service.query<Product>(
        sql, 
  ...
  async getProductById(id: number) {
    try {
      ...
      const result = await this.d1Service.query<Product>(sql, [validatedId], Schemas.Product);
  ...
  async getProductByBarcode(barcode: string) {
    try {
      ...
      const result = await this.d1Service.query<Product>(sql, [barcode.trim()], Schemas.Product);
  ...
  async createProduct(productData: ProductCreate) {
    try {
      ...
      if (validatedData.barcode) {
        const existingProduct = await this.d1Service.findOne(
          'products',
  ...
  async updateProduct(id: number, productData: ProductUpdate) {
    try {
      ...
      if (validatedData.barcode && validatedData.barcode !== existingProduct.barcode) {
        const duplicateProduct = await this.d1Service.findOne(
          'products',
          { barcode: validatedData.barcode }
        );
      ...
      if (validatedData.stock_quantity !== undefined) {
        await this.d1Service.execute(
          'UPDATE inventory SET quantity_available = ?, updated_at = ? WHERE product_id = ?',
          [validatedData.stock_quantity, new Date().toISOString(), validatedId]
        );
      }
  ...
  async deleteProduct(id: number) {
    try {
      ...
      const orderItems = await this.d1Service.query(
      ...
      if (hasOrders) {
        ...
        return createSuccessResponse(
          { id: validatedId, status: 'inactive' },
          'Product deactivated successfully (has order history)'
        );
      } else {
  ...
  async updateStock(id: number, quantity: number, operation: 'set' | 'add' | 'subtract' = 'set') {
    try {
      ...
      if (operation === 'set') {
        ...
        // Get current stock
        const currentStock = await this.d1Service.query<{ quantity_available: number }>(
      ...
      // Update inventory
      const updateResult = await this.d1Service.execute(
        `UPDATE inventory 
         SET quantity_available = ?, updated_at = ?
         WHERE product_id = ?`,
        [newQuantity, new Date().toISOString(), validatedId]
      );
      ...
      return createSuccessResponse(
        { id: validatedId, quantity: newQuantity, operation },
  ...
  async getCategories() {
    try {
      const result = await this.d1Service.query<{ id: number; name: string; description?: string }>(
