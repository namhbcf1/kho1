...
export interface InventoryMovement {
  id: string;
  productId: number;
  type: 'sale' | 'purchase' | 'adjustment' | 'reservation' | 'release';
  quantity: number;
...
class InventoryEventManager {
  ...
  subscribe(productId: string, callback: (event: any) => void): () => void {
...
class StockReservationManager {
  ...
  async createReservation(params: z.infer<typeof ReservationSchema>): Promise<StockReservation | null> {
    ...
    try {
      ...
      await this.kv.put(
        ...
        JSON.stringify({ productId: validated.productId, quantity: validated.quantity }),
        { expirationTtl: validated.expirationMinutes * 60 }
      );
...
export class OptimizedInventoryService {
  private eventManager = new InventoryEventManager();
  private reservationManager: StockReservationManager;
   ...
   */
  async getInventory(productId: number): Promise<InventoryItem | null> {
    try {
      const product = await this.db.prepare(`
  ...
  private async performStockUpdate(
    productId: number,
    quantityChange: number,
    type: 'sale' | 'purchase' | 'adjustment',
    userId: number,
  ...
  async getLowStockAlerts(): Promise<Array<{
    ...
    return lowStockItems.results.map((item: any) => ({
      productId: item.id,
   ...
   * Record inventory movement
   */
  private async recordInventoryMovement(movement: Omit<InventoryMovement, 'id' | 'timestamp'>): Promise<void> {
   ...
   */
  private async checkReorderLevel(productId: number, currentStock: number): Promise<void> {
    const product = await this.db.prepare(`
