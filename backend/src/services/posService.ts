...
class POSService {
  async getActiveProducts(db: D1Database): Promise<POSResponse> {
    try {
  ...
  async searchProducts(db: D1Database, query: string): Promise<POSResponse> {
    try {
  ...
  async getProductByBarcode(db: D1Database, barcode: string): Promise<POSResponse> {
    try {
  ...
  async createOrder(db: D1Database, orderData: CreateOrderData): Promise<POSResponse> {
    try {
      ...
      await db.batch([
        ...
        ...(orderData.customerId ? [
          db.prepare(`
            ...
            WHERE id = ?
          `).bind(
            Math.floor(orderData.total / 1000), // 1 point per 1000 VND
            orderData.total,
            orderData.customerId
          )
        ] : []),
  ...
  async getOrderById(db: D1Database, orderId: string): Promise<POSResponse> {
    try {
      // Get order details
  ...
  async updateOrderStatus(db: D1Database, orderId: string, status: string, userId: string): Promise<POSResponse> {
    try {
      await db
        .prepare(`
          UPDATE orders 
  ...
  async processRefund(db: D1Database, orderId: string, refundData: any): Promise<POSResponse> {
    try {
  ...
  async getCustomers(db: D1Database, search?: string): Promise<POSResponse> {
    try {
      let query = `
  ...
  async searchCustomers(db: D1Database, query: string): Promise<POSResponse> {
