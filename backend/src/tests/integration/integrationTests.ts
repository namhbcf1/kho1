...
.test('VAT Declaration Generation', async () => {
  ...
  const declaration = vatService.generateVATDeclaration(
    '2024-01',
    '0123456789',
    transactions
  );
...
.test('API Endpoint Integration', async () => {
  ...
  const processAPIRequest = async (request: any) => {
    // Validate request
    ...
    for (const item of items) {
      const vatResult = vatService.calculateItemVAT(
        item.productId,
        item.price,
        item.quantity,
        'individual'
