...
interface RateLimitRule {
  ...
  blockDuration?: number; // seconds to block after limit exceeded
}
...
export const createRateLimitMiddleware = (type: keyof RateLimitConfig = 'default') => {
...
export const createBruteForceProtection = () => {
  return async (c: Context<{ Bindings: Env }>, next: Next) => {
    try {
      ...
      let ipAttempts: number[] = ipData ? JSON.parse(ipData) : [];
...
export const trackFailedLogin = async (c: Context<{ Bindings: Env }>, next: Next) => {
  await next();
