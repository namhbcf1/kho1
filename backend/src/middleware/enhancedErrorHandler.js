...
export class AppError extends Error {
    ...
    details;
    correlationId;
    constructor(message, type = ErrorType.INTERNAL_ERROR, statusCode = 500, isOperational = true, details, correlationId) {
        super(message);
        this.type = type;
...
export function createErrorResponse(error, c, correlationId) {
...
export function asyncHandler(fn) {
    return (...args) => {
        return Promise.resolve(fn(...args)).catch((error) => {
            ...
            if (!(error instanceof AppError)) {
                throw new AppError(error.message || 'Unexpected error occurred', ErrorType.INTERNAL_ERROR, 500, false);
            }
            throw error;
        });
    };
