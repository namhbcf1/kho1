export const checkInventoryMiddleware = async (c, next) => {
    try {
...
export const updateInventoryAfterSale = async (c, orderItems) => {
    try {
        const db = c.env.DB;
      ...
      `).bind(crypto.randomUUID(), item.productId, -item.quantity, // Negative for sale
            item.price, c.get('orderId')).run();
...
export const reserveInventory = async (c, orderItems, orderId) => {
    try {
        const db = c.env.DB;
        for (const item of orderItems) {
            await db.prepare(`
      ...
      `).bind(crypto.randomUUID(), item.productId, orderId, item.quantity).run();
        }
    }
    catch (error) {
...
export const releaseInventoryReservation = async (c, orderId) => {
    try {
        const db = c.env.DB;
...
const createLowStockAlert = async (c, productId, productName, currentStock, minStock) => {
    try {
        const db = c.env.DB;
    ...
    `).bind(productId).first();
      ...
      `).bind(crypto.randomUUID(), productId, `Product "${productName}" is below minimum stock level`, currentStock, minStock).run();
        }
    }
    catch (error) {
