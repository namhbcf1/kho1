// Vietnamese tax calculation middleware
import { Context, Next } from 'hono';

export const taxMiddleware = async (c: Context, next: Next) => {
  try {
...
export const calculateTax = (orderData: any, taxSettings: any) => {
  let subtotal = 0;
...
export const getTaxSettings = async (c: Context) => {
  try {
    const db = c.env.DB;
    ...
    for (const setting of settings.results || []) {
      try {
        taxSettings[setting.key as keyof typeof taxSettings] = JSON.parse(setting.value);
      } catch (parseError) {
...
export const isProductTaxExempt = async (c: Context, productId: string) => {
  try {
    const taxSettings = await getTaxSettings(c);
...
export const calculateItemTax = (item: any, taxSettings: any) => {
  const itemTotal = item.price * item.quantity;
  const itemDiscount = item.discountAmount || 0;
...
export const generateTaxReport = async (c: Context, startDate: string, endDate: string) => {
  try {
    const db = c.env.DB;
