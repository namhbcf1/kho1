// Loyalty points processing middleware
import { Context, Next } from 'hono';

export const loyaltyMiddleware = async (c: Context, next: Next) => {
  try {
...
export const calculateLoyaltyPoints = async (c: Context, orderData: any) => {
  try {
    const db = c.env.DB;
...
export const awardLoyaltyPoints = async (c: Context, loyaltyCalculation: any, orderId: string) => {
  try {
    const db = c.env.DB;
    ...
    if (loyaltyCalculation.pointsToEarn > 0) {
      ...
      `).bind(
        crypto.randomUUID(),
        loyaltyCalculation.customerId,
        orderId,
        loyaltyCalculation.pointsToEarn,
        `Earned points from order ${orderId}`
      ).run();
    }
    ...
    if (loyaltyCalculation.pointsToRedeem > 0) {
      ...
      `).bind(
        ...
        loyaltyCalculation.customerId,
        orderId,
        -loyaltyCalculation.pointsToRedeem,
        `Redeemed points for order ${orderId}`
      ).run();
    }
...
export const getLoyaltySettings = async (c: Context) => {
  try {
    const db = c.env.DB;
    ...
    for (const setting of settings.results || []) {
      try {
        loyaltySettings[setting.key as keyof typeof loyaltySettings] = JSON.parse(setting.value);
      } catch (parseError) {
...
export const getTierById = async (c: Context, tierId: string) => {
  try {
    const db = c.env.DB;
...
export const checkTierUpgrade = async (c: Context, customerId: string) => {
  try {
    const db = c.env.DB;
    ...
    if (newTierId && newTierId !== customer.tier_id) {
      ...
      `).bind(
        ...
        customerId,
        'Tier Upgrade',
        'Congratulations! You have been upgraded to a new loyalty tier.'
      ).run();
    }
  } catch (error) {
