...
export const zalopayHandler = new Hono()
    ...
    try {
      ...
      `).bind(crypto.randomUUID(), paymentId, zalopayRequest.app_id, zalopayRequest.app_trans_id, zalopayRequest.app_user, zalopayRequest.app_time, zalopayRequest.amount, zalopayRequest.description, zalopayRequest.bank_code, zalopayRequest.callback_url, zalopayRequest.embed_data, mac, result.order_url || '', result.qr_code || '').run();
        return c.json({
            success: true,
            paymentId,
            appTransId,
    ...
    try {
      ...
      `).bind(refundId, payment.order_id, -amount, 'pending', gatewayConfig.id, refundResult.refund_id || '', JSON.stringify(refundResult), amount).run();
