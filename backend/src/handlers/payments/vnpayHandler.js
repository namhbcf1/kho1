...
export const vnpayHandler = new Hono()
    ...
    try {
      ...
      `).bind(crypto.randomUUID(), paymentId, vnpParams.vnp_TmnCode, vnpParams.vnp_Amount, vnpParams.vnp_Command, vnpParams.vnp_CreateDate, vnpParams.vnp_CurrCode, vnpParams.vnp_IpAddr, vnpParams.vnp_Locale, vnpParams.vnp_OrderInfo, vnpParams.vnp_OrderType, vnpParams.vnp_ReturnUrl, vnpParams.vnp_TxnRef, vnpParams.vnp_Version, secureHash).run();
        return c.json({
            success: true,
            paymentId,
            paymentUrl,
    ...
    try {
      ...
      `).bind(params.vnp_TxnRef).first();
        ...
        WHERE payment_id = ?
      `).bind(params.vnp_ResponseCode, params.vnp_TransactionNo, params.vnp_BankCode, params.vnp_BankTranNo, params.vnp_CardType, params.vnp_PayDate, payment.id).run();
    ...
    try {
      ...
      `).bind(refundId, payment.order_id, -amount, 'pending', gatewayConfig.id, refundResult.vnp_TransactionNo || '', JSON.stringify(refundResult), amount).run();
