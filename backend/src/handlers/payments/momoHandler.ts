...
export const momoHandler = new Hono()
  .post('/create', zValidator('json', momoCreateSchema), async (c) => {
    try {
      ...
      `).bind(
        crypto.randomUUID(), paymentId, gatewayConfig.merchant_id, paymentId, requestId,
        amount, orderInfo, returnUrl, momoRequest.ipnUrl, 'vi',
        extraData, requestType, signature, result.qrCodeUrl || '',
        result.deeplink || '', result.payUrl || ''
      ).run();
  ...
  .post('/refund', async (c) => {
    try {
      ...
      `).bind(
        ...
        refundResult.transId || '', JSON.stringify(refundResult), amount
      ).run();
