...
import { OrderCreationData, TransactionService } from '../../services/database/transactionService';
...
orderRoutes.post('/', 
  ...
  async (c) => {
    try {
      ...
      const transactionOrderData: OrderCreationData = {
        orderNumber,
...
orderRoutes.post('/reserve-inventory',
  enhancedAuth(),
  requirePermission('order:create'),
  zValidator('json', reserveInventorySchema),
  async (c) => {
    try {
      ...
      for (const item of items) {
        const result = await transactionService.reserveInventory(
          item.productId,
          item.quantity,
          orderId,
          expiresInMinutes
        ...
        if (!result.success) {
          ...
          return c.json(createErrorResponse(
            result.error || 'Inventory reservation failed',
            'RESERVATION_ERROR'
          ), 400);
        }
...
orderRoutes.post('/release-inventory/:orderId',
  enhancedAuth(),
  ...
  async (c) => {
    try {
      ...
      if (!result.success) {
        return c.json(createErrorResponse(
          result.error || 'Failed to release inventory reservation',
          'RELEASE_ERROR'
        ), 400);
      }
...
orderRoutes.post('/cleanup-reservations',
  enhancedAuth(),
  ...
  async (c) => {
    try {
      ...
      if (!result.success) {
        return c.json(createErrorResponse(
          result.error || 'Failed to cleanup expired reservations',
          'CLEANUP_ERROR'
        ), 500);
      }
