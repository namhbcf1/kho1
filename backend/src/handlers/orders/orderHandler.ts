...
export class OrderHandler {
  ...
  async createOrder(c: Context<{ Bindings: Env }>) {
    try {
      ...
      `).bind(
        orderId, customer_id || null, subtotal, tax, discount + loyaltyDiscount,
        loyalty_points_used, total, payment_method, notes || null,
      ...
      if (customer_id) {
        ...
        if (customer) {
          ...
          const newPoints = (customer.loyalty_points as number) - loyalty_points_used + pointsEarned;
          ...
          if (loyalty_points_used > 0) {
            ...
            `).bind(
              crypto.randomUUID(), customer_id, loyalty_points_used,
  ...
  async getOrders(c: Context<{ Bindings: Env }>) {
    try {
  ...
  async getOrder(c: Context<{ Bindings: Env }>) {
    try {
  ...
  async updateOrderStatus(c: Context<{ Bindings: Env }>) {
    try {
      const id = c.req.param('id');
  ...
  async deleteOrder(c: Context<{ Bindings: Env }>) {
    try {
      const id = c.req.param('id');
  ...
  async getOrderStats(c: Context<{ Bindings: Env }>) {
    try {
  ...
  async getRecentOrders(c: Context<{ Bindings: Env }>) {
    try {
