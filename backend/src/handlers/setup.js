...
setupRoutes.post('/create-tables', async (c) => {
    ...
    `);
        return c.json(createSuccessResponse({ message: 'Database tables created successfully' }, 'Tables created'));
    }
    catch (error) {
        console.error('Error creating tables:', error);
        return c.json(createErrorResponse('Failed to create database tables: ' + (error instanceof Error ? error.message : 'Unknown error'), 'SETUP_ERROR'), 500);
    }
});
// Create admin user
setupRoutes.post('/create-admin', async (c) => {
    try {
        ...
        if (!email || !password || !name) {
            return c.json(createErrorResponse('Email, password, and name are required', 'VALIDATION_ERROR'), 400);
        }
        // Check if admin already exists
        const existingUser = await c.env.DB.prepare('SELECT id FROM users WHERE email = ? OR role = ?').bind(email, 'admin').first();
        if (existingUser) {
            return c.json(createErrorResponse('Admin user already exists', 'USER_EXISTS'), 400);
        }
        // Hash password
    ...
    `).bind(userId, email, hashedPassword, name, role, 1).run();
      ...
      `).bind(product.id, product.name, product.description, product.price, product.cost, product.stock_quantity, product.barcode, product.sku, product.category_id).run();
      ...
      `).bind(product.id, product.stock_quantity).run();
        ...
        return c.json(createErrorResponse('Failed to create admin user: ' + (error instanceof Error ? error.message : 'Unknown error'), 'ADMIN_CREATION_ERROR'), 500);
    }
});
...
setupRoutes.get('/status', async (c) => {
    ...
    `).all();
        ...
        return c.json(createSuccessResponse({
            ...
            databaseReady: tables.results?.length > 0 && adminCount?.count > 0
        }, 'Database status retrieved'));
    }
    catch (error) {
        console.error('Error checking status:', error);
        return c.json(createErrorResponse('Failed to check database status: ' + (error instanceof Error ? error.message : 'Unknown error'), 'STATUS_ERROR'), 500);
    }
});
export { setupRoutes };
