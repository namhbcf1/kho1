...
export class POSHandler {
    ...
    async quickSale(c) {
        try {
            ...
            const { items, payment_method = 'cash', customer_phone, discount = 0, received_amount, notes } = saleData;
      ...
      `).bind(orderId, customer_id, subtotal, tax, discount, total, payment_method, notes || null, currentTime, currentTime).run();
        ...
        `).bind(customer_id).first();
          ...
          `).bind(newPoints, newTotalSpent, currentTime, currentTime, customer_id).run();
            ...
            `).bind(crypto.randomUUID(), customer_id, pointsEarned, `Quick Sale ${orderId}`, currentTime).run();
                    }
                }
            }
    ...
    async searchProducts(c) {
        ...
        ORDER BY 
          ...
          name ASC
        LIMIT ?
      `).bind(`%${query}%`, `%${query}%`, `%${query}%`, `${query}%`, query, `${query}%`, limit).all();
            const formattedProducts = products.results?.map(product => ({
                ...product,
                formatted_price: formatVND(product.price),
    ...
    async getCustomerByPhone(c) {
      ...
      `).bind(phone).first();
            ...
            const formattedCustomer = {
                ...customer,
                formatted_total_spent: formatVND(customer.total_spent)
            };
      ...
      `).bind(date).all();
            return c.json(createSuccessResponse({
                ...
                hourly_breakdown: hourlyStats.results?.map(hour => ({
                    ...hour,
                    formatted_revenue: formatVND(hour.hourly_revenue)
                })) || []
    ...
    async getRecentTransactions(c) {
      ...
      `).bind(limit).all();
            const formattedTransactions = transactions.results?.map(transaction => ({
                ...transaction,
                formatted_total: formatVND(transaction.total)
            })) || [];
      ...
      `).bind(orderId).all();
            const receipt = {
                ...
                items: items.results?.map(item => ({
                    ...item,
                    formatted_price: formatVND(item.price),
                    formatted_total: formatVND(item.total)
                })) || [],
                ...
                formatted_discount: formatVND(order.discount),
