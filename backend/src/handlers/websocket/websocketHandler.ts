...
export const websocketHandler = {
  ...
  async broadcast(channel: string, data: any, c: Context) {
  ...
  async updateDashboardMetrics(c: Context) {
    try {
      const db = c.env.DB;
      const now = new Date();
  ...
  async updateInventory(productId: string, newStock: number, c: Context) {
    try {
  ...
  async updateOrderStatus(orderId: string, newStatus: string, c: Context) {
    try {
      const orderUpdate = {
        orderId,
        newStatus,
  ...
  async trackUserActivity(userId: string, activity: string, c: Context) {
    try {
      const userActivity = {
        userId,
        activity,
...
function handleWebSocketMessage(client: WebSocketClient, data: string, c: Context) {
  try {
...
function handleWebSocketClose(client: WebSocketClient, c: Context) {
  activeConnections.delete(client.id);
