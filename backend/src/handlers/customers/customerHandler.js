...
import { calculateLoyaltyTier, vietnameseLoyaltyTiers } from '../../utils/vietnamese';
export class CustomerHandler {
    ...
    async getCustomers(c) {
      ...
      `;
            ...
            const customers = customersResult.results?.map(customer => ({
                ...
                calculated_tier: calculateLoyaltyTier(customer.total_spent),
    ...
    async createCustomer(c) {
      ...
      `).bind(customerId, name, phone, email || null, address || null, currentTime, currentTime, currentTime, currentTime).run();
    ...
    async addLoyaltyPoints(c) {
      ...
      `).bind(customerId).first();
            ...
            const newTier = calculateLoyaltyTier(customer.total_spent);
            await this.db.prepare(`
        UPDATE customers 
      ...
      `).bind(crypto.randomUUID(), customerId, points, reason || 'Manual addition', new Date().toISOString()).run();
            const updatedCustomer = await this.db.prepare(`
        SELECT * FROM customers WHERE id = ?
      `).bind(customerId).first();
    ...
    async redeemLoyaltyPoints(c) {
      ...
      `).bind(crypto.randomUUID(), customerId, points, reason || 'Points redemption', new Date().toISOString()).run();
            const updatedCustomer = await this.db.prepare(`
        SELECT * FROM customers WHERE id = ?
      `).bind(customerId).first();
