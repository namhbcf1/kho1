...
export const loginHandler = new Hono()
    .post('/', bruteForceProtection, zValidator('json', loginSchema), async (c) => {
    try {
        ...
        const refreshToken = jwt.sign({ userId: user.id, type: 'refresh' }, c.env.JWT_REFRESH_SECRET || c.env.JWT_SECRET, { expiresIn: '7d' });
