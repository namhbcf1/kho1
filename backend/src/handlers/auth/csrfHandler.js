...
export const csrfHandler = new Hono()
    .get('/csrf-token', apiRateLimit, async (c) => {
    try {
        // Generate CSRF token
        ...
        await c.env.CACHE.put(`csrf:${tokenHash}`, JSON.stringify(csrfData), { expirationTtl: 3600 } // 1 hour
        );
        return c.json({
            success: true,
            data: {
    ...
    .post('/csrf-verify', apiRateLimit, async (c) => {
    try {
