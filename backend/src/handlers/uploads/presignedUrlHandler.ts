// Presigned URL generation handler
...
export const presignedUrlHandler = {
  ...
  async generateDownloadUrl(c: Context) {
    try {
      const r2 = c.env.R2;
      if (!r2) {
        return c.json({
  ...
  async confirmUpload(c: Context) {
    try {
      ...
      `).bind(
        ...
        pendingUpload.key,
        object.size,
        pendingUpload.content_type,
        pendingUpload.folder,
        pendingUpload.uploaded_by
      ).run();
  ...
  async cleanupExpiredUploads(c: Context) {
    try {
      const db = c.env.DB;
      const r2 = c.env.R2;
