export const imageHandler = {
    async uploadImage(c) {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))
      `).bind(imageId, image.name, fileName, key, image.size, image.type, folder, JSON.stringify(variants), c.get('user')?.id || 'anonymous').run();
            return c.json({
                success: true,
                data: {
                    image: {
      ...
      `).bind(id, id).first();
            ...
            if (size && imageRecord.variants) {
                try {
                    ...
                    const variant = variants.find((v) => v.type === size);
                    if (variant) {
                        key = variant.key;
                    }
                }
    ...
    async listImages(c) {
      ...
      `).bind(...params).first();
            ...
            const imagesWithVariants = (images.results || []).map((image) => ({
                ...image,
